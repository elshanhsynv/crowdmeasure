...
import com.example.crowdmeasure.domain.model.ThroughputTestConfig
...
object ThroughputTester {
    ...
    fun run(
        ...
        if (!config.enabled) return ThroughputResult()

        val safeConfig = config.normalized()
        if (safeConfig.maxTotalBytes <= 0L) return ThroughputResult()
    ...
    private fun ThroughputTestConfig.normalized(): ThroughputTestConfig {
        return copy(
    ...
    private fun ThroughputTestConfig.isAllowedUrl(url: HttpUrl): Boolean {
        if (requireHttps && url.scheme != "https") return false
        return true
    }
