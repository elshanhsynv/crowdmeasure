...
internal class CallPhoneStateReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        when (intent.getStringExtra(TelephonyManager.EXTRA_STATE)) {
            ...
            TelephonyManager.EXTRA_STATE_IDLE -> { wasRinging = false; CallSamplingService.stop(context, CallSource.CELLULAR) }
    ...
    private companion object { var wasRinging = false }
}

internal class VoipCallMonitor(private val context: Context, private val settings: CallsSettingsStore) {
    ...
    fun start() {
        ...
        job = scope.launch {
            ...
            while (isActive) {
                ...
                if (active && !detected) { delay(2_000); if (context.getSystemService(AudioManager::class.java)?.mode.isVoipMode()) { detected = true; requestStart() } }
                if (!active && detected) { delay(3_000); if (!context.getSystemService(AudioManager::class.java)?.mode.isVoipMode()) { detected = false; CallSamplingService.stop(context, CallSource.VOIP_GENERIC) } }
                delay(1_000)
            }
        }
    }
    fun stop() { job?.cancel(); job = null; if (detected) CallSamplingService.stop(context, CallSource.VOIP_GENERIC); detected = false; scope.launch { settings.setVoipActive(false) } }
    private suspend fun requestStart() {
        val requirements = context.requirements()
        if (!requirements.canStart) settings.recordMissed(requirements.failureCode()) else runCatching { CallSamplingService.start(context, CallType.UNKNOWN, CallSource.VOIP_GENERIC) }.onFailure { settings.recordMissed(CallRunCode.FOREGROUND_SERVICE_FAILED) }
    }
}

private fun Int?.isVoipMode() = this == AudioManager.MODE_IN_COMMUNICATION || (Build.VERSION.SDK_INT >= 33 && this == AudioManager.MODE_COMMUNICATION_REDIRECT)
...
internal class CallSamplingService : Service() {
    ...
    private suspend fun startSampling(rt: InstalledCallsRuntime, type: CallType, source: CallSource) {
        active?.let { session ->
    ...
    private fun createChannel() {
        ...
        getSystemService(NotificationManager::class.java)?.createNotificationChannel(NotificationChannel(CHANNEL, rt.config.notificationChannelName, NotificationManager.IMPORTANCE_LOW))
    }
    ...
    companion object {
        ...
        fun stop(context: Context, source: CallSource) { context.startService(Intent(context, CallSamplingService::class.java).setAction(ACTION_STOP).putExtra(EXTRA_SOURCE, source.name)) }
    }
}
private fun Intent.type() = getStringExtra("type")?.let { runCatching { CallType.valueOf(it) }.getOrNull() } ?: CallType.UNKNOWN
private fun Intent.source() = getStringExtra("source")?.let { runCatching { CallSource.valueOf(it) }.getOrNull() } ?: CallSource.UNKNOWN
